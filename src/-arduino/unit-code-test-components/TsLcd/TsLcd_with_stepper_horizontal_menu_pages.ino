...
int tmp_dispenseTime = dispense_time;

void display_4_setDispenseTime()
{
  islistening = true;
  ...
  tmp_dispenseTime = dispense_time;
  
  tft.setTextColor(WHITE, DARKGREY);  
  tft.setTextSize(6);     // System font is 8 pixels.  ht = 8*2=16 wdth = 5/6 * size
  tft.setCursor(160, 111);
  tft.print(tmp_dispenseTime);
  up_dispenseTime_btn.initButton(&tft, 180, 75, 150, 60, BLACK, NAVY, WHITE, "UP", 2);
  up_dispenseTime_btn.drawButton(false);
  down_dispenseTime_btn.initButton(&tft, 180, 170,150, 60, BLACK, NAVY, WHITE, "DOWN", 2);
...
void display_4_setDispenseTime_listener()
{
  ...
  tft.setCursor(90, 111);
  tft.print(tmp_dispenseTime);
  dispense_time = tmp_dispenseTime;
